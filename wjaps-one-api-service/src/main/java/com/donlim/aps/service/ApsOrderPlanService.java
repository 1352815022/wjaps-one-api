...
public class ApsOrderPlanService extends BaseEntityService<ApsOrderPlan> {
   ...
   private ApsOrderDao apsOrderDao;
    ...
    /**已停用
     * 根据SCM送货计划明细自动生成预排产计划
     * 1.生成排产主计划
     * 2.根据bom拆分组件找出对应的子件工单
     * 3.刷新内排单为已下达
     */
    /*@Transactional(rollbackFor = Exception.class)
    public void generatePlan(){
        List<OrderAndScm> mainOrders = apsOrderDao.findWithScmAndU9Produce();
        List<ApsOrderPlan> plans = new ArrayList<>();
        List<ApsOrder> apsOrders = new ArrayList<>();
        Map<String,List<ApsOrderPlanDetail>> details = new HashMap<>();

        //数据整理
        for (OrderAndScm mainOrder : mainOrders) {
            ScmXbDelivery scmXbDelivery = mainOrder.getScmXbDelivery();
            //排除找不到送货计划明细的工单
            if ( null == scmXbDelivery.getDeliveryPlans() || scmXbDelivery.getDeliveryPlans().size() ==0 ){
                continue;
            }
            //组件/能在SCM上直接找到的工单
            ApsOrder apsOrder = mainOrder.getApsOrder();
            U9Material material = mainOrder.getU9Material();
            int fixedLt = NumberUtils.getBigDecimalValue(material.getFixedLt()).intValue();

            ApsOrderPlan plan =generatorPlan(apsOrder);

            plan.setScmDeliveryDate(scmXbDelivery.getDeliveryStartDate());
            //开始、结束日期减去物料提前期
            try {
                plan.setStartDate(plan.getScmDeliveryDate().minusDays(fixedLt));
            }catch (NullPointerException e){
                e.printStackTrace();
            }
            plan.setEndDate(scmXbDelivery.getDeliveryEndDate().minusDays(fixedLt));
            List<ScmXbDeliveryPlan> deliveryPlans = scmXbDelivery.getDeliveryPlans();
            List<ApsOrderPlanDetail> detail = new ArrayList<>();
            BigDecimal awaitQty = apsOrder.getOrderQty();
            for (ScmXbDeliveryPlan deliveryPlan : deliveryPlans) {
                ApsOrderPlanDetail planDetail  = new ApsOrderPlanDetail();
                planDetail.setPlanDate(deliveryPlan.getDeliveryDate().minusDays(fixedLt));
                awaitQty = awaitQty.subtract(deliveryPlan.getQty());
                planDetail.setPlanQty(deliveryPlan.getQty());
                detail.add(planDetail);
            }
            plan.setAwaitQty(awaitQty);
            detail = detail.stream().sorted(Comparator.comparing(ApsOrderPlanDetail::getPlanDate)).collect(Collectors.toList());
            //刷新待排单
            refreshApsOrder(apsOrder,plan);
            apsOrders.add(apsOrder);
            plans.add(plan);
            details.put(apsOrder.getId(),detail);


            //子件
            List<U9Bom> minorComponents = bomService.getMinorComponents(apsOrder.getMaterialId());
            List<Long> minorComponentIds = minorComponents.stream().map(b -> Long.valueOf(b.getMaterial().getId())).collect(Collectors.toList());
            List<OrderAndU9> minorOrdersAndU9 = produceOrderDao.queryInnerOrderBySoIdAndMaterialIdIn(mainOrder.getU9ProduceOrder().getSoId(), minorComponentIds);
            for (OrderAndU9 minor : minorOrdersAndU9) {
                ApsOrder minorOrder = minor.getApsOrder();
                int minorFixedLt = NumberUtils.getBigDecimalValue(minor.getU9Material().getFixedLt()).intValue();
                ApsOrderPlan minorPlan = generatorPlan(minorOrder);
                minorPlan.setStartDate(plan.getStartDate().plusDays(fixedLt).minusDays(minorFixedLt));
                minorPlan.setEndDate(plan.getEndDate().plusDays(fixedLt).minusDays(minorFixedLt));
                List<ApsOrderPlanDetail> minorDetail = new ArrayList<>();
                for (ApsOrderPlanDetail planDetail : detail) {
                    ApsOrderPlanDetail minorPlanDetail = new ApsOrderPlanDetail();
                    BigDecimal scale = planDetail.getPlanQty().divide(plan.getPlanQty(), 4, BigDecimal.ROUND_HALF_UP);
                    minorPlanDetail.setPlanQty(minorPlan.getPlanQty().multiply(scale).setScale(0,BigDecimal.ROUND_UP));
                    //避免出现总日排产数量>生产数量
                    if (minorPlan.getAwaitQty().compareTo(minorPlanDetail.getPlanQty())>=0){
                        minorPlan.setAwaitQty(minorPlan.getAwaitQty().subtract(minorPlanDetail.getPlanQty()));
                    }else{
                        minorPlanDetail.setPlanQty(plan.getAwaitQty());
                        minorPlan.setAwaitQty(BigDecimal.ZERO);
                    }
                    minorPlanDetail.setPlanDate(planDetail.getPlanDate().plusDays(fixedLt).minusDays(minorFixedLt));
                    minorDetail.add(minorPlanDetail);
                }

                //刷新待排单状态
                refreshApsOrder(minorOrder,minorPlan);
                apsOrders.add(minorOrder);
                plans.add(minorPlan);
                details.put(minorOrder.getId(),minorDetail);
            }
        }
        //保存
        this.save(plans);
        for (ApsOrderPlan plan : plans) {
            List<ApsOrderPlanDetail> orderPlanDetails = details.get(plan.getOrderId());
            for (ApsOrderPlanDetail orderPlanDetail : orderPlanDetails) {
                orderPlanDetail.setPlanId(plan.getId());
            }
            apsOrderPlanDetailDao.save(orderPlanDetails);
        }
        apsOrderDao.save(apsOrders);
     ...
     * @return
     */
  /*  public ApsOrderPlan generatorPlan(ApsOrder apsOrder) {
        ApsOrderPlan plan = new ApsOrderPlan();
        plan.setOrderId(apsOrder.getId());
        plan.setMaterialId(apsOrder.getMaterialId());
        plan.setMaterialCode(apsOrder.getMaterialCode());
        plan.setMaterialName(apsOrder.getMaterialName());
        plan.setMaterialSpec(apsOrder.getMaterialSpec());
        plan.setPlanQty(apsOrder.getOrderQty());
        plan.setOrderDate(apsOrder.getOrderDate());
        plan.setAwaitQty(apsOrder.getOrderQty());
        return plan;
    }*/

    public void refreshApsOrder(ApsOrder apsOrder, ApsOrderPlan plan) {
        apsOrder.setStatus(OrderStatusType.Released);
        apsOrder.setProduceQty(plan.getPlanQty());
        apsOrder.setPlanFinishDate(plan.getEndDate());
        apsOrder.setNoPlanQty(plan.getAwaitQty());
    }
